...
function SolarSystemScale({ probes }: { probes: DeepSpaceProbe[] }) {
  return (
    <div style={{
      ...
      {/* Scale bar — logarithmic + collision-stagger so no probes overlap */}
      {(() => {
        const LOG_MIN   = Math.log10(0.04)
        const LOG_MAX   = Math.log10(170)
        const toPct     = (au: number) =>
          Math.min(((Math.log10(Math.max(au, 0.04)) - LOG_MIN) / (LOG_MAX - LOG_MIN)) * 100, 97)

        // Build probe list with pct positions
        const items = probes.map(p => ({ probe: p, pct: toPct(p.distanceFromSun), row: 0 }))

        // Sort by pct, then assign rows to avoid overlap (min 7% gap)
        items.sort((a, b) => a.pct - b.pct)
        for (let i = 0; i < items.length; i++) {
          for (let j = 0; j < i; j++) {
            if (items[j].row === items[i].row && Math.abs(items[i].pct - items[j].pct) < 8) {
              items[i].row = items[j].row + 1
            }
          }
        }
        const maxRow    = Math.max(...items.map(x => x.row))
        const rowHeight = 56  // px per row
        const totalH    = (maxRow + 1) * rowHeight + 40  // track line at bottom

        return (
          <div style={{ position: 'relative', height: `${totalH}px`, marginBottom: '8px' }}>
            ...
            {([0.1, 1, 10, 100] as number[]).map(au => {
              const pct = ((Math.log10(au) - LOG_MIN) / (LOG_MAX - LOG_MIN)) * 100
              ...
              return (
                <div key={au} style={{
                  ...
                  <div style={{ width: '1px', height: '8px', background: 'rgba(255,255,255,0.18)', margin: '0 auto 3px' }} />
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', color: 'rgba(240,244,250,0.4)', whiteSpace: 'nowrap' }}>
                    {au} AU
                  </span>
                </div>
              )
            ...
            <div style={{
              ...
              borderRadius: '50%',
              background:   '#f5c518',
              boxShadow:    '0 0 18px #f5c51899',
              transform:    'translateX(-50%)',
              zIndex:       2,
            }} />

            {/* Probe markers with vertical stagger */}
            {items.map(({ probe, pct, row }) => {
              const color      = probeColor(probe.id)
              const bottomPos  = 26 + (maxRow - row) * rowHeight  // higher row = lower on screen (closer to track)
              return (
                <div key={probe.id} style={{
                  ...
                  <span style={{ fontSize: '16px', lineHeight: 1 }}>{probeIcon(probe.id)}</span>
                  <div style={{
                    width:        '10px',
                    height:       '10px',
                    borderRadius: '50%',
