...
// Server component — reads hero config from Supabase
export async function HeroSection() {
  const hero = await getHeroConfigPublic()

  // Fallback defaults if no config saved yet
  const badge       = hero?.badge       || 'NASA · Saturn Mission'
  const title       = hero?.title       || "Cassini's Legacy: New Data Reveals Ocean Activity Deep Within Enceladus"
  const excerpt     = hero?.excerpt     || "Researchers reanalysing Cassini's final passes have found evidence of active hydrothermal vents at Enceladus's seafloor."
  const readTime    = hero?.readTime    || '10 min read'
  const category    = hero?.category    || 'Research'
  const articleSlug = hero?.articleSlug || ''
    ...
    <section style={{ position: 'relative', zIndex: 10, minHeight: '100vh', display: 'flex', alignItems: 'center', paddingTop: '60px' }}>
      <div style={{ maxWidth: '1380px', margin: '0 auto', width: '100%', padding: '60px 24px 40px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '40px', alignItems: 'center' }}>
          ...
          {/* RIGHT — FEATURED CARD (dynamic) */}
          <Link href={cardHref} style={{ textDecoration: 'none', display: 'block' }}>
            <div style={{ background: '#10151c', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '16px', overflow: 'hidden', transition: 'border-color 0.2s' }}>
              {/* Card graphic header */}
              <div style={{ height: '200px', position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(145deg, #080f20 0%, #0c1830 50%, #070e1c 100%)' }}>
                ...
                <div style={{ position: 'absolute', top: '14px', left: '14px', fontFamily: 'DM Mono, monospace', fontSize: '9px', letterSpacing: '0.2em', textTransform: 'uppercase', padding: '4px 10px', borderRadius: '3px', background: 'rgba(59,158,255,0.12)', border: '1px solid rgba(59,158,255,0.25)', color: '#3b9eff' }}>
                  Featured Story
                </div>
              </div>

              {/* Card content — from Supabase */}
