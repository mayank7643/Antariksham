...
// Auto-fetch latest featured article as fallback
async function getFeaturedArticle() {
  const { data, error } = await supabaseAdmin()
    .from('articles')
    .select('title, slug, excerpt, reading_time, article_type, categories:article_categories(categories(name))')
    .eq('featured', true)
    .eq('status', 'published')
...
export async function HeroSection() {
  // Priority: Admin hero config > latest featured article > hardcoded fallback
  ...
  const badge    = hero?.badge
    || (featuredArticle ? 'Featured Story' : 'NASA · Saturn Mission')
  ...
  const readTime = hero?.readTime
    || (featuredArticle?.reading_time ? `${featuredArticle.reading_time} min read` : '10 min read')
