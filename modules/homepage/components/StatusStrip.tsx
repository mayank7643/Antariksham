...
export function StatusStrip() {
    ...
    try {
      ...
      if (dsRes.status === 'fulfilled' && dsRes.value.ok) {
        ...
        if (v1?.distanceFromSun) {
          ...
          next.voyagerStatus   = `Interstellar · ${v1.velocity ? Math.round(v1.velocity) + ' km/s' : '17 km/s'}`
        }
      }
  ...
  const items = [
    { icon: '🛸', label: 'ISS Position',   value: data.issSpeed,        sub: '● Live tracking',      subColor: '#34d897' },
    { icon: '🚀', label: 'Next Launch',     value: 'View Schedule',      sub: 'Launch Library 2',     subColor: 'rgba(240,244,250,0.7)', href: '/live/launches' },
    { icon: '🌌', label: 'NASA APOD',       value: 'Today\'s Image',     sub: 'Updated daily',        subColor: 'rgba(240,244,250,0.7)', href: '/live/apod' },
    { icon: '🛰️', label: 'Voyager 1',       value: data.voyagerDistance, sub: data.voyagerStatus,     subColor: 'rgba(240,244,250,0.7)', href: '/live/deep-space/voyager-1' },
    { icon: '🌍', label: 'Deep Space',      value: '5 Probes',           sub: 'Live telemetry',       subColor: 'rgba(240,244,250,0.7)', href: '/live/deep-space' },
  ]
      ...
      <div style={{ display: 'flex', alignItems: 'stretch', padding: '0 24px', minWidth: 'max-content' }}>
        {items.map((item, i, arr) => {
          const inner = (
            <div
              key={item.label}
              style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '20px 28px 20px 0', marginRight: '28px', borderRight: i < arr.length - 1 ? '1px solid rgba(255,255,255,0.08)' : 'none', whiteSpace: 'nowrap', flexShrink: 0, cursor: item.href ? 'pointer' : 'default' }}
            >
              ...
              <div>
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', letterSpacing: '0.18em', textTransform: 'uppercase', color: 'rgba(240,244,250,0.5)', marginBottom: '2px' }}>{item.label}</div>
                <div style={{ fontFamily: 'var(--font-sans)', fontSize: '16px', fontWeight: 500, color: '#ffffff', marginBottom: '2px' }}>{item.value}</div>
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', color: item.subColor }}>{item.sub}</div>
              </div>
            </div>
          )
