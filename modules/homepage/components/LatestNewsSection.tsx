...
function getCategoryColor(categories: { slug: string }[] | undefined): string {
  if (!categories?.length) return '#3b9eff'
  return CATEGORY_COLORS[categories[0].slug] || '#3b9eff'
}
...
function LeadCard({ article }: { article: ArticleCard }) {
  ...
  const categoryLabel = article.categories?.[0]?.name || 'Space'
  return (
