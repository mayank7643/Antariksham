...
export async function generateMetadata(
  ...
  const seo         = article.seo_metadata as { meta_title?: string; meta_description?: string } | null
  const title       = translation?.title   || seo?.meta_title   || 'Antariksham'
  const description = translation?.excerpt || seo?.meta_description || ''
...
export default async function HindiArticlePage(
  ...
  const author      = article.authors as { id?: string; name?: string; avatar?: string } | null
            ...
            <div style={{
              ...
              {author && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  {author.avatar && (
                    <img src={author.avatar} alt={author.name} style={{ width: 28, height: 28, borderRadius: '50%', objectFit: 'cover' }} />
                  )}
                  <span style={{ color: 'rgba(240,244,250,0.7)' }}>{author.name}</span>
                </div>
              )}
