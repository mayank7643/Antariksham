...
async function fetchHorizonsVectors(spkid: string): Promise<{
  ...
  try {
    ...
    // Line 2 = X Y Z (AU), Line 3 = VX VY VZ (AU/d)
    if (lines.length < 3) return null

    const xyzParts = lines[1].trim().split(/\s+/)
    const vParts   = lines[2].trim().split(/\s+/)

    const x  = parseFloat(xyzParts[1] || '0')
    const y  = parseFloat(xyzParts[2] || '0')
    const z  = parseFloat(xyzParts[3] || '0')
    const vx = parseFloat(vParts[1]   || '0')
    const vy = parseFloat(vParts[2]   || '0')
    const vz = parseFloat(vParts[3]   || '0')
